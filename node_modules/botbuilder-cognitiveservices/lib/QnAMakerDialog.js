...
var QnAMakerDialog = (function (_super) {
    ...
    QnAMakerDialog.prototype.replyReceived = function (session, recognizeResult) {
        ...
        var noMatchMessage = "No match found!  Please re-enter your question or type 'menu' to return to the main menu.";
        if (!recognizeResult) {
            ...
            this.recognize(context, function (error, result) {
                try {
                    if (!error) {
                        _this.invokeAnswer(session, result, threshold, "Ask me anything");
                    }
                }
                catch (e) {
    ...
    QnAMakerDialog.prototype.respondFromQnAMakerResult = function (session, qnaMakerResult) {
        session.send('**Q.** %s \n\n **A.** %s',
                ...
                qnaMakerResult.answers[0].answer); // Added by DC
        //session.send(session.message.text); // Added by DC
